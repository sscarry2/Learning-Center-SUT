...
export const roomSorter = (roomList, buildingNumber) => {
  
  let copiedList = roomList.slice(0)
  
  // filter list of rooms to those on the given building
  let filteredList = copiedList.filter(room => {
    return room.building === buildingNumber
  })
  
  // function to sort rooms numerically by their building number
