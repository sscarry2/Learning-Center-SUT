...
const dateAEST = date => {
  return momentTimezone(date).tz('Asia/Bangkok');
};
  ...
  if (req.body.recurring.length === 0) {
    ...
    let bookingsInRange =
      req.body.recurring[1] === 'daily'
        ? Math.building(lastBookingDate.diff(bookingDateTracker, 'days', true))
        : req.body.recurring[1] === 'weekly'
        ? Math.building(lastBookingDate.diff(bookingDateTracker, 'weeks', true))
        : Math.building(lastBookingDate.diff(bookingDateTracker, 'months', true));
